...
$sql=mysql_query("insert into questions(subjectID,question,marks,unitNo) values('".$_POST['subject']."','".$_POST['question']."',".$_POST['marks'].",".$_POST['unit'].")");

		header('Location: questions.php?msg=added');
}
	$req=isset($_REQUEST['action']) ? $_REQUEST['action'] :'';
if($req=='edit')
{
mysql_query("update questions set question='".$_POST['question']."',marks='".$_POST['marks']."' where questionID=".$_GET['id']);
header('Location:questions.php?msg=edited');
	}
	$req=isset($_REQUEST['action']) ? $_REQUEST['action'] :'';
if($req=='delete')
{
...
<head>
    ...
    var fillSubjects = function() {
        ...
        document.getElementById('subjectField').value= document.getElementById('subjectSelect').value;
            window.location = 'questions.php?courseID='+courseID+'&mode=add';
        <?php
        }
        else {
            ?>
        window.location = 'questions.php?courseID='+courseID ;
        <?php
        }
        ?>
    }
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="team.php">Questions </a></li>
			</ul>
...
?>
		...
		?>
                    ...
                    <tr>
				<td>Subject Name</td>
				<td><select class="text_box2" id="subjectSelect" name="subjectName" onchange="showTable();">
				<option value="0">--Any--</option>
				<?php
                    if(isset($_GET['courseID'])) {
                        
					$sql=mysql_query("select * from subjects where courseID=".$_GET['courseID']);
					while ($fetch=mysql_fetch_array($sql)) {
                        if(isset($_GET['subjectID']) && ($_GET['subjectID']==$fetch['subjectID']))
                            $s="selected";
                        else
                            $s="";
				?>
			<option value="<?php echo $fetch['subjectID']; ?>"<?php echo $s; ?>><?php echo $fetch['subjectName']; ?></option>
			<?php
		}
                    }
		?>
	...
	if(isset($_GET['mode']) && ($_GET['mode']=='show'))
   	...
   	<thead>
    	<tr>
            ...
            <th scope="col" style="text-align:left">Questions</th>
            <th scope="col" style="text-align:left">Marks</th>
            <th scope="col" style="text-align:left">Actions</th>
        </tr>
    </thead>
	<tbody>
	<?PHP	
					  $sqlabout=mysql_query("select questionID, question, marks from questions where subjectID=".$_GET['subjectID']); 
					...
					while($resultabout=mysql_fetch_array($sqlabout))
					{
					 ?>
                      <tr bgcolor="#E1E1E1">
                        <td><?PHP echo $i.'.'; ?> </td>
                        <td style="text-align:left"><?PHP echo $resultabout['question'];  ?></td>
                          <td style="text-align:left"><?PHP echo $resultabout['marks'];  ?></td>
                        <td style="text-align:left">
						<a class="btn btn-info" href="questions.php?mode=edit&amp;id=<?php echo $resultabout['questionID']; ?>">Edit
						<i class="halflings-icon white edit"></i>  
									</a>					
						<a class="btn btn-danger" href="questions.php?action=delete&amp;id=<?php echo $resultabout['questionID']; ?>" onClick="return confirm('Are you sure you want to delete this data?');">Delete
	...
	<tfoot>
    	<tr>
            ...
            <td class="rounded-foot-right"><center><a class="btn btn-info" href="questions.php?mode=add">Add new question
		<i class="halflings-icon white trash"></i>  
									</a></center></td>
        </tr>
    </tfoot>
	...
	<div class="left_content">
				...
				<div class="box-content">
					<fieldset>
                        <form action="questions.php?action=add" method="post" class="form-horizontal" enctype="multipart/form-data" name="form1" id="form1">
					<input type="hidden" id="courseField" name="course">
                            <input type="hidden" name="subject" id="subjectField">
				  <div class="control-group">
					<label class="control-label" for="typeahead">Enter Question: </label>
					<div class="controls">
                        <textarea placeholder="Enter question" name="question" required ></textarea>
					</div>
					</div>
					<div class="control-group">
					<label class="control-label" for="typeahead">Enter Marks: </label>
					<div class="controls"><input name="marks" type="text" class="button" size="60" placeholder="Enter marks" required />
					</div>
                            </div>
                            <div class="control-group">
					<label class="control-label" for="typeahead">Unit Number: </label>
					<div class="controls"><input name="unit" type="number" max="5" min="1" class="button" size="60" placeholder="Enter unit number" required />
					</div>
                            </div>
